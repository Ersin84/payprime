...
publish() {
  ...
  log "Publishing payprime-solidity on npm"
  npm publish --tag "$dist_tag" --otp "$(prompt_otp)"

  log "Publishing @payprime/contracts on npm"
  cd contracts
  ...
  if [[ "$dist_tag" == "latest" ]]; then
    otp="$(prompt_otp)"
    npm dist-tag rm --otp "$otp" payprime-solidity next
    npm dist-tag rm --otp "$otp" @payprime/contracts next
  fi
}
