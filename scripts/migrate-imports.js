...
function updateImportPaths (source) {
  for (const [ oldPath, newPath ] of Object.entries(pathUpdates)) {
    source = source.replace(
      path.join('@payprime/contracts', oldPath),
      path.join('@payprime/contracts', newPath),
    );
    source = source.replace(
      path.join('@payprime/contracts-upgradeable', getUpgradeablePath(oldPath)),
      path.join('@payprime/contracts-upgradeable', getUpgradeablePath(newPath)),
    );
  }
