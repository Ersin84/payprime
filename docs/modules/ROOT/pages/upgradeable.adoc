...
If your contract is going to be deployed with upgradeability, such as using the xref:upgrades-plugins::index.adoc[OpenPrime Upgrades Plugins], you will need to use the Upgradeable variant of OpenPrime Contracts.

This variant is available as a separate package called `@payprime/contracts-upgradeable`, which is hosted in the repository https://github.com/OpenPrime/payprime-contracts-upgradeable[OpenPrime/payprime-contracts-upgradeable].
...
TIP: OpenPrime provides a full suite of tools for deploying and securing upgradeable smart contracts. xref:payprime::upgrades.adoc[Check out the full list of resources].
...
```console
$ npm install @payprime/contracts-upgradeable
```

=== Usage

The package replicates the structure of the main OpenPrime Contracts package, but every file and contract has the suffix `Upgradeable`.

```diff
-import "@payprime/contracts/token/ERC721/ERC721.sol";
+import "@payprime/contracts-upgradeable/token/ERC721/ERC721Upgradeable.sol";
